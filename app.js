...
const { spawn } = require('child_process'); // 서버 재시작용
...
app.post('/submit-save', upload.single('cafe-image'), (req, res) => {
    ...
    csvWriterInstance.writeRecords(addrows)
        .then(() => {
            ...
            // 서버 재시작 명령어 ----- 되는지 봐야됨
            spawn(process.argv[0], process.argv.slice(1), { // 새로운 node.js 프로세스 실행
                detached: true,
                stdio: 'ignore'  // 각 프로세스 분리
            }).unref();
...
app.post('/api/favorites', (req, res) => {
    ...
    console.log('Received cafeId:', cafeId);

    if (!userId || !cafeId) {
        return res.status(400).json({ success: false, message: 'User ID and Cafe ID are required' });
    }

    const favorite = { userId, cafeId };

    const exists = favorites.some(fav => fav.userId === userId && fav.cafeId === cafeId);
    if (!exists) {
        ...
        favorites = favorites.filter(fav => !(fav.userId === userId && fav.cafeId === cafeId));
        fs.writeFileSync(favoritesPath, JSON.stringify(favorites, null, 2));
...
app.get('/api/favorites/:userId', (req, res) => {
    ...
    console.log('Received userId for favorites:', userId);
...
app.delete('/api/favorites/:userId', (req, res) => {
    ...
    console.log('Deleting all favorites for userId:', userId);

    if (!userId) {
        return res.status(400).json({ success: false, message: 'User ID is required' });
    }

    favorites = favorites.filter(fav => fav.userId !== userId);
    fs.writeFileSync(favoritesPath, JSON.stringify(favorites, null, 2));
